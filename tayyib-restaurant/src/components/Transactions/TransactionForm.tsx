...
export default function TransactionForm() {
  ...
  // Determine if we only show expenses (MD)
  ...
  const { register, control, handleSubmit, watch } = useForm<FormData>({
    defaultValues: {
      ...
      previous_balance: 0,
      notes: '',
      purchase_items: isMD ? [] : [{ item_name: '', quantity: 0, unit: 'pieces', unit_price: 0 }],
    ...
    try {
      const { data: txn, error } = await supabase
        ...
        .insert({
          ...
          pos_total: isMD ? 0 : data.pos_total,
          meal_tickets: isMD ? 0 : data.meal_tickets,
          cash_received: isMD ? 0 : data.cash_received,
          previous_balance: isMD ? 0 : data.previous_balance,
      ...
      // Insert purchase items (only for non-MD)
      if (!isMD) {
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <h2 className="text-xl font-bold mb-4">
        {isMD ? 'Add Expense' : 'New Transaction'}
      </h2>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 bg-white p-6 rounded-lg shadow">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {/* Sales fields – only for Manager / MD if you later decide to allow them */}
          {!isMD && (
            <>
              <div>
        ...
        {/* Purchase items – only for Manager */}
        {!isMD && (
          <div>
        ...
        <div>
          ...
          <button
            type="button"
            onClick={() => addExpense({ description: '', amount: 0, category: 'supplies' })}
        ...
        <Button type="submit" className="w-full">
          {isMD ? 'Save Expense' : 'Save Transaction'}
        </Button>
      </form>
    </div>
  );
