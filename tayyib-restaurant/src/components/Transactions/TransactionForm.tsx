...
export default function TransactionForm() {
  ...
  useEffect(() => {
    async function init() {
      ...
      // Always fetch the current last balance, even in edit (optional)
      await fetchLastBalance();
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Remove old items/expenses if editing
      if (isEdit) {
      ...
      // Insert purchase items (only for Manager)
      if (!isMD) {
        ...
        console.log('Inserting items:', items); // debug
        if (items.length > 0) {
          const { error: itemsError } = await supabase.from('purchase_items').insert(items as any);
          if (itemsError) {
      ...
      console.log('Inserting expenses:', expenses); // debug
      if (expenses.length > 0) {
        const { error: expError } = await supabase.from('expenses').insert(expenses as any);
        if (expError) {
          console.error('Expenses insert error:', expError);
          throw expError;
        }
